...
#include "zxing/CameraImageWrapper.h"

#include <QtCore>

using namespace zxing;

TcZXing::TcZXing(QObject* parent)
    : QObject(parent)
{
...
QString TcZXing::decodeImage(QImage image)
{
    Ref<Result> result;
    Q_EMIT decodingStarted();
    ...
    try {
        ...
        Q_EMIT tagFound(string);
        Q_EMIT decodingFinished(true);
        return string;
    }
...
QString TcZXing::decodeImageQML(const QUrl &imageUrl)
{
    return decodeSubImageQML(imageUrl);
}

QString TcZXing::decodeSubImageQML(const QUrl &imageUrl,
