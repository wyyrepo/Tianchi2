...
TcColorWheel::TcColorWheel(QWidget* parent)
    : QWidget(parent)
    , initSize(200,200)
    , mouseDown(false)
    , margin(0)
...
QColor TcColorWheel::color()
{
    return current;
}

void TcColorWheel::setColor(const QColor &color)
{
...
QColor TcColorWheel::posColor(const QPoint &point)
{
...
QSize TcColorWheel::sizeHint () const
{
    return QSize(height(),height());
}
QSize TcColorWheel::minimumSizeHint () const
{
    return initSize;
}

void TcColorWheel::mousePressEvent(QMouseEvent *event)
{
    lastPos = event->pos();
...
void TcColorWheel::mouseMoveEvent(QMouseEvent *event)
{
    lastPos = event->pos();
    if( !mouseDown ) return;
...
void TcColorWheel::mouseReleaseEvent(QMouseEvent *)
{
    mouseDown = false;
    inWheel = false;
    inSquare = false;
...
void TcColorWheel::resizeEvent(QResizeEvent *event)
{
...
void TcColorWheel::paintEvent(QPaintEvent *)
{
...
void TcColorWheel::drawWheelImage(const QSize &newSize)
{
...
void TcColorWheel::drawSquareImage(const int &hue)
{
...
void TcColorWheel::drawIndicator(const int &hue)
{
...
void TcColorWheel::drawPicker(const QColor &color)
{
    QPainter painter(&wheel);
    painter.setRenderHint(QPainter::Antialiasing);
    QPen pen;
...
void TcColorWheel::composeWheel()
{
...
void TcColorWheel::hueChanged(const int &hue)
{
...
void TcColorWheel::svChanged(const QColor &newcolor)
{
    int hue = current.hue();
